...
I've given you an installation CD you can use. If anything ever happens to it, you can re-create one from http://github.com/matthewtodd/hcts-bootstrapper.

Boot the installation CD, follow through the process, and reboot the system.
...
Then, log in and:
 ...
 git clone git://github.com/matthewtodd/hcts.git /srv/chef
 cd /srv/chef
 git submodule init
 ...
 cd /etc/chef
 cp /src/chef/config/attributes.json .
 vi attributes.json
...
Then, if you're restoring data from a backup (say on a USB stick):

 rsnapshot-restore /media/UDISK/latest.tar.gz

And that's it!

You can add additional *administrative* users -- NOT customers -- with this commands:

 sudo adduser henry

If henry should also have permission to use sudo, you would instead:

 sudo adduser henry
 sudo adduser henry admin
...
To restore a backup from the hard disk:
 ...
 sudo -i
 cd /var/backups/rsnapshot/hourly.0
 rsnapshot-restore
